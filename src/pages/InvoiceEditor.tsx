...
import { generateQrBillSvg, convertSvgToPngDataUrl } from '../services/qrBillService';
...
const InvoiceEditor = () => {
  ...
  useEffect(() => {
      let isMounted = true;
      if (qrCodeSvg) {
          convertSvgToPngDataUrl(qrCodeSvg).then(pngDataUrl => {
              if (isMounted) setQrCodePng(pngDataUrl);
          });
      } else {
          setQrCodePng('');
      }
  ...
  const invoicePdfDocument = <InvoicePDF invoiceData={invoiceData} qrCodePng={qrCodePng} />;
    ...
    <div>
        {isZoomModalOpen && (
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4" onClick={() => setIsZoomModalOpen(false)}>
                <div className="relative bg-gray-900 rounded-md shadow-2xl h-[95vh] w-[95vw] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
                    <div className="flex justify-end p-2"><button onClick={() => setIsZoomModalOpen(false)} className="z-10 p-1.5 bg-gray-800 text-white rounded-full hover:bg-gray-700 transition-colors" aria-label="Vorschau schliessen"><X size={24} /></button></div>
                    <div className="flex-grow min-h-0"><PDFViewer width="100%" height="100%" style={{ border: 'none' }}>{invoicePdfDocument}</PDFViewer></div>
                </div>
            </div>
        )}
        ...
        <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-2">
                 <PDFDownloadLink document={invoicePdfDocument} fileName={`Rechnung-${invoiceData.unstructuredMessage || invoiceData.id}.pdf`} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-300 flex items-center gap-2">
                    {({ loading }) => (loading ? 'Generiere...' : <><Download size={16} />PDF herunterladen</>)}
                </PDFDownloadLink>
